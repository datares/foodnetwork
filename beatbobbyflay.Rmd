...
ggplot(top10appearances, aes(reorder(alljudges, Freq), Freq))+ geom_bar(stat = "identity", fill = "#b82329") +
  coord_flip() + ggtitle("Top 10 Most Appearances for Second Round Judges on Beat Bobby Flay") + xlab("Second Round Judge") +ylab("Number of Appearances on Beat Bobby Flay") + scale_y_continuous(labels = comma, limits = c(0, 20), breaks = seq(0, 20, by = 5)) + geom_text(aes(label = scales :: comma(Freq, accuracy = 1)), hjust = -0.1, colour = "black")
```
